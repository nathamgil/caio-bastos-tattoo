...
(function () {
  ...
  var CHAVE = 'cb_agendamentos_demo';
  ...
  function demoBloqueios() {
    try { return JSON.parse(localStorage.getItem('cb_bloqueios_demo') || '[]'); }
    catch (e) { return []; }
  }
  ...
  function demoSemeia() {
    try { if (localStorage.getItem('cb_demo_semeado_v2')) return; } catch (e) { return; }
    ...
    demoGrava(lista);
    try { localStorage.setItem('cb_demo_semeado_v2', '1'); } catch (e) {}
  }
  ...
  try {
    var salvo = JSON.parse(localStorage.getItem('cb_cliente') || 'null');
  ...
  form.addEventListener('submit', function (ev) {
    ...
    API.criar({
      ...
      try { localStorage.setItem('cb_cliente', JSON.stringify({ nome: nome, tel: campoTel.value })); } catch (e) {}
